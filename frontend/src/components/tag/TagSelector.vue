    ...
    >
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md mx-4 max-h-[80vh] flex flex-col">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">选择标签</h2>
          <button
            @click="handleClose"
        ...
        <div class="flex-1 overflow-y-auto p-6">
          <div v-if="availableTags.length === 0" class="text-center text-gray-400 dark:text-gray-500 py-8">
            <p>暂无其他标签</p>
          ...
          <div v-else class="space-y-2">
            <label
              ...
              :key="tag._id"
              class="flex items-center gap-3 p-3 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer transition-colors"
            >
              <input
                type="checkbox"
                :value="tag._id"
              ...
              <div class="flex-1 min-w-0">
                <div class="font-medium text-gray-900 dark:text-gray-100">{{ tag.name }}</div>
                <p v-if="tag.prompt" class="text-xs text-gray-500 dark:text-gray-400 mt-0.5 truncate">
                  {{ tag.prompt }}
                </p>
              </div>
            </label>
