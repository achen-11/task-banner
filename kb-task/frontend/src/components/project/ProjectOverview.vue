<template>
  <div class="space-y-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
      <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100 hover:shadow-md transition-shadow duration-200">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-sm font-medium text-gray-600">总任务数</h3>
        ...
        <p class="text-3xl font-bold text-gray-900 mb-2">{{ project?.taskCount || 0 }}</p>
        <div class="text-xs text-gray-500">项目总任务量</div>
      </div>

      <div class="bg-white rounded-xl shadow-sm p-6 border border-blue-100 hover:shadow-md transition-shadow duration-200">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-sm font-medium text-gray-600">进行中</h3>
        ...
        <p class="text-3xl font-bold text-blue-600 mb-2">{{ pendingTaskCount }}</p>
        <div class="text-xs text-gray-500">待完成任务</div>
      </div>

      <div class="bg-white rounded-xl shadow-sm p-6 border border-green-100 hover:shadow-md transition-shadow duration-200">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-sm font-medium text-gray-600">已完成</h3>
        ...
        <p class="text-3xl font-bold text-green-600 mb-2">{{ project?.completedTaskCount || 0 }}</p>
        <div class="text-xs text-gray-500">已完成任务</div>
      </div>

      <div class="bg-white rounded-xl shadow-sm p-6 border border-purple-100 hover:shadow-md transition-shadow duration-200">
        <div class="flex items-center justify-between mb-4">
        ...
        <p class="text-3xl font-bold text-purple-600 mb-2">{{ completionRate }}%</p>
        <div class="w-full bg-gray-200 rounded-full h-2">
          <div class="bg-purple-600 h-2 rounded-full transition-all duration-300" :style="{ width: `${completionRate}%` }"></div>
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <!-- 项目描述 -->
      <div v-if="project?.description" class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
      ...
      <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
        ...
        <div class="space-y-4">
          ...
          <div class="flex justify-between items-center">
            ...
            <span class="text-gray-900 font-medium">{{ project?.memberCount || 0 }} 人</span>
          </div>
          <div class="flex justify-between items-center">
            <span class="text-gray-600">创建时间</span>
            <span class="text-gray-900 font-medium">{{ formatDate(project?.createdAt) }}</span>
          </div>
          <div class="flex justify-between items-center">
            <span class="text-gray-600">最后更新</span>
            <span class="text-gray-900 font-medium">{{ formatDate(project?.updatedAt) }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <button class="p-4 bg-green-50 hover:bg-green-100 rounded-lg text-center transition-colors duration-200">
          ...
          <div class="text-sm font-medium text-green-900">导出任务</div>
        </button>
        <button class="p-4 bg-purple-50 hover:bg-purple-100 rounded-lg text-center transition-colors duration-200">
          ...
          <div class="text-sm font-medium text-purple-900">成员管理</div>
        </button>
        <button class="p-4 bg-orange-50 hover:bg-orange-100 rounded-lg text-center transition-colors duration-200">
          ...
          <div class="text-sm font-medium text-orange-900">项目设置</div>
        </button>
      </div>
    </div>
  </div>
